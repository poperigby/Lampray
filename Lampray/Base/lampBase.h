...
class lampTypes {
  public:
     ...
     *
     * This class provides custom functionality for string manipulation and
     * conversion.
     */
    class lampString {
      private:
      ...
      public:
        ...
        /**
         * @brief Custom operator to concatenate a lampString with a C-style
         * string (const char*).
         *
         * @param other The C-style string to concatenate with.
         * @return A new lampString containing the concatenated result.
         */
        lampString operator+(const char *other) const { return {data + other}; }
        ...
        /**
         * @brief Custom operator to concatenate a std::string with a
         * lampString.
         *
         ...
         * @return A new lampString containing the concatenated result.
         */
        friend lampString operator+(const std::string &str1,
                                    const lampString &str2) {
            return {str1 + str2.data};
        }

        /**
         * @brief Custom operator to concatenate a lampString with a
         * std::string.
         *
        ...
        /**
         * @brief Custom operator to assign a C-style string (const char*) to a
         * lampString.
         *
         ...
         * @return true if this lampString is less than the other, false
         * otherwise.
         */
        ...
        /**
         * @brief Custom operator to convert a lampString to a C-style string
         * (const char*).
         *
        ...
        /**
         * @brief Custom operator to convert a lampString to a
         * std::filesystem::path.
         *
         * @return A std::filesystem::path representing the content of the
         * lampString.
         */
         ...
         *
         * This function interprets "1" or "true" as `true`, and anything else
         * as `false`.
         *
         * @return true if the string is "1" or "true," false otherwise.
         */
        bool as_bool() { return (data == "1" || data == "true"); }

        /**
         * @brief Get C string equivalent
         *
         * @return  A pointer to the c-string representation of the lampString
         * object's value.
         */
    ...
    /**
     * @brief A custom color class designed for use in the Lampray framework,
     * representing a color with alpha (RGB-A).
     *
     * This class provides functionality to work with colors in RGBA format and
     * supports conversion to/from hex strings.
     */
    class lampHexAlpha {
      public:
        /**
         * @brief Constructs a lampHexAlpha with the default color (black with
         * full alpha).
         */
         ...
         *
         * @param hexColor The hex color string (e.g., "RRGGBB-AA") to
         * initialize the lampHexAlpha with.
         */
         ...
         */
        operator ImVec4() const { return color_; }
         ...
         */
        operator std::string() const { return ImVec4ToHexString(color_); }
      ...
      private:
         ...
         *
         * @param hexColor The hex color string in the format "RRGGBB-AA" to
         * convert.
    ...
    /**
     * @brief A custom class for representing return values with an associated
     * reason.
     *
     * This class is designed for use in the Lampray framework to encapsulate
     * return values along with an optional reason.
     */
    class lampReturn {
      public:
        int returnClause; ///< The return clause indicating the outcome of an
                          ///< operation.
        lampString returnReason = ""; ///< The optional return reason.

        /**
         * @brief Constructs a lampReturn with a return clause and an optional
         * return reason.
         *
         * @param returnClause The return clause indicating the outcome of an
         * operation.
        ...
        /**
         * @brief Constructs a lampReturn with a return clause and no return
         * reason.
         *
         * @param returnClause The return clause indicating the outcome of an
         * operation.
        ...
        /**
         * @brief Default constructor for a lampReturn with no return clause and
         * no return reason.
         */
        lampReturn() {}
         ...
         */
        operator bool() const { return returnClause == 1; }
         ...
         */
        operator int() const { return returnClause; }
         ...
         */
        operator lampString() const { return returnReason; }
    ...
    /**
     * @brief A structure representing identification information for game
     * control.
     *
     * This structure is used to provide readable and shorthand names for game
     * identification purposes.
     */
    struct lampIdent {
        lampTypes::lampString
            ReadableName; ///< A human-readable name for the game.
        lampTypes::lampString
            ShortHand; ///< A shorthand or abbreviated name for the game.

        /**
         * @brief Constructs a lampIdent with a readable name and a shorthand
         * name.
         *
         * @param readableName The human-readable name for the game.
         * @param shortHand The shorthand or abbreviated name for the game.
         */
        lampIdent(lampTypes::lampString readableName,
  ...
  public:
    ...
    class Profile {
      public:
         ...
         *
         * @param inputString The input string from which the value will be
         * removed.
         * @param valueToRemove The value to remove from the input string.
         */
        static void removeValue(std::string &inputString,
                                const std::string &valueToRemove) {
...
/**
 * @brief A singleton class for logging messages and displaying warnings in the
 * Lampray framework.
 *
 * This class provides a centralized logging system and a mechanism to display
 * warning messages. It is implemented as a singleton to ensure a single
...
class lampLog {
  public:
    ...
    /**
     * @brief Enumeration representing different warning levels for log
     * messages.
     */
    enum warningLevel {
        LOG,     ///< Log message.
    ...
    // Passthrough log.
    lampTypes::lampReturn pLog(lampTypes::lampReturn data,
                               warningLevel warningLevel = warningLevel::LOG,
                               bool pop = false,
                               errorCode errorCode = errorCode::LMP_UNK) {
     ...
     */
    void log(lampTypes::lampString data,
             warningLevel warningLevel = warningLevel::LOG, bool pop = false,
             errorCode errorCode = errorCode::LMP_UNK) {
        ...
        const std::string filename =
            ((std::string)std::getenv("HOME")) + "/.lamp/lamp.log";
        ...
        if (pop) {
            switch (warningLevel) {
            case WARNING:
                Lamp::Core::lampNotification::getInstance()
                    .pushWarningNotification(ping);
                break;
            case ERROR:
                Lamp::Core::lampNotification::getInstance()
                    .pushErrorNotification(ping);
                break;
            }
    ...
    [[maybe_unused]] bool showWarns() {
        for (auto it = poplist.begin(); it != poplist.end();) {
            if (!poplist.empty()) {
                ...
                ImGui::Text(
                    "If an error persists please create an issue on GitHub.");
  ...
  private:
    /**
     * @brief Private constructor for the lampLog class to enforce singleton
     * pattern.
     */
    lampLog() {}
};
 ...
 *
 * The `lampExecutor` class allows you to execute a provided code function
 * asynchronously on a list of mods. It keeps track of the total number of items
 * and the count of finished items.
 */
class lampExecutor {
  public:
    /**
     * @brief Constructs a `lampExecutor` object with the provided code
     * function.
     *
     * @param code The code function to execute on each mod.
     */
    lampExecutor(std::function<void(const lampMod::Mod *)> code)
        : code_(code), itemCount(0), finishedCount(0) {}

    int itemCount;     ///< The total number of items to execute.
    int finishedCount; ///< The count of items that have finished execution.

    /**
     * @brief Executes the provided code function on a list of mods
     * asynchronously.
     *
     * This method creates a separate thread for each mod in the list and
     * executes the provided code function on it. It also keeps track of the
    ...
    void execute(const std::vector<lampMod::Mod *> &data, bool detach) {
        lampLog::getInstance().log("ModList Executor: Starting",
                                   lampLog::warningLevel::LOG);
        itemCount = data.size();
    ...
    void waitForCompletion() {
        std::unique_lock<std::mutex> lock(mutex_);
        condition_.wait(lock, [this]() { return finishedCount == itemCount; });
    }

  private:
    std::function<void(const lampMod::Mod *)>
        code_;         ///< The code function to execute.
    std::mutex mutex_; ///< Mutex for thread synchronization.
    std::condition_variable
        condition_; ///< Condition variable for thread synchronization.
};

/**
 * @brief A utility class for sequencing and executing lambda expressions in
 * named queues.
 *
 * The `LampSequencer` class allows you to add lambda expressions to named
 * queues and execute them sequentially. It logs the results of the execution,
 * including success, failure, and critical failure.
 */
class LampSequencer {
  public:
    /**
     ...
     *
     * @param queueName The name of the queue to which the lambda should be
     * added.
     * @param lambda The lambda expression to be added to the queue.
     */
    static void add(const std::string &queueName,
                    std::function<lampTypes::lampReturn()> lambda) {
     ...
     *
     * This method executes lambda expressions in the specified queue one by one
     * and logs the results. Execution stops on critical failure (returnClause
     * == -1) to prevent further execution.
     *
     * @param queueName The name of the queue to run.
     * @return A lampReturn object indicating the overall result of the
     * execution.
     */
    static lampTypes::lampReturn run(const std::string &queueName) {
        lampLog::getInstance().log("Queue '" + queueName + "': Starting",
                                   lampLog::warningLevel::LOG);
        lampTypes::lampReturn result;
        while (!getQueue(queueName).empty()) {
            std::function<lampTypes::lampReturn()> lambda =
                getQueue(queueName).front();
  ...
  private:
     ...
     *
     * This method returns the queue associated with the provided name or
     ...
     */
    static std::queue<std::function<lampTypes::lampReturn()>> &
    getQueue(const std::string &queueName) {
        static std::unordered_map<
            std::string, std::queue<std::function<lampTypes::lampReturn()>>>
            queues;
     ...
     *
     * This method logs the result of lambda execution in the specified queue,
     * indicating success, failure, or critical failure.
     *
     * @param queueName The name of the queue.
     * @param result The result of lambda execution.
     */
    static void logResult(const std::string &queueName,
                          const lampTypes::lampReturn &result) {
        std::string logMessage = "Queue '" + queueName + "': ";
        if (result.returnClause == 0) {
            logMessage += "Failed - " + (std::string)result.returnReason;
            lampLog::getInstance().log(logMessage, lampLog::warningLevel::ERROR,
                                       true);
            ...
            logMessage +=
                "Critically Failed - " + (std::string)result.returnReason;
            lampLog::getInstance().log(logMessage, lampLog::warningLevel::ERROR,
                                       true);
        }
    }
...
class OverlayBuilder {
  ...
  public:
    ...
    lampTypes::lampReturn create(std::string gameTarget,
                                 std::string configTarget,
                                 std::string workingTarget) {
        std::filesystem::path gamePath(gameTarget);
        std::filesystem::path configPath(configTarget);
        if (std::filesystem::exists(workingTarget) &&
            std::filesystem::is_directory(workingTarget) &&
            std::filesystem::exists(gamePath) &&
            std::filesystem::is_directory(gamePath) &&
            std::filesystem::exists(configPath) &&
            std::filesystem::is_directory(configPath)) {
            // Rename the games folder and store a copy of the original name to
            // be used for the merge.
            std::string managedString = std::string("Lampray Managed - ") +
                                        gamePath.filename().string();
            std::filesystem::path MergedPath =
                gamePath.parent_path() / managedString;
