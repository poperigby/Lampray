...
void Lamp::Core::lampMenu::RunMenus() {
    ...
    if (Lamp::Core::lampControl::getInstance().inDeployment) {
        ImGui::Begin(
            "DEPLOYMENT", NULL,
            Lamp::Core::lampConfig::getInstance().DefaultWindowFlags());
        ImGui::Text("%s", Lamp::Core::lampControl::getInstance()
                              .deploymentStageTitle.c_str());
        ImGui::Text("%s", (std::to_string(Lamp::Core::lampControl::getInstance()
                                              .deplopmentTracker.first) +
                           "/" +
                           std::to_string(Lamp::Core::lampControl::getInstance()
                                              .deplopmentTracker.second))
                              .c_str());
        ImGui::End();
    } else {
        switch (currentMenu) {
        case LICENCE_MENU:
            if (Lamp::Core::lampConfig::getInstance()
                    .lampFlags["showIntroMenu"]
                    .as_bool()) {
                IntroMenu();
            } else {
...
void Lamp::Core::lampMenu::IntroMenu() {
    ...
    ImGui::Begin("Requires a title, but we've disabled it.", NULL,
                 Lamp::Core::lampConfig::getInstance().DefaultWindowFlags());
    DefaultMenuBar();
    ImGui::Separator();
    ...
    size = ImGui::CalcTextSize(lampLang::LS("LAMPRAY_LONGNAME")).x +
...
void Lamp::Core::lampMenu::ModMenu() {
    ...
    ImGuiStyle &style = ImGui::GetStyle();
    float size = ImGui::CalcTextSize(lampLang::LS("LAMPRAY_DRAGANDDROP")).x +
                 style.FramePadding.x * 2.0f;
    float avail = ImGui::GetContentRegionAvail().x;
    ...
    size = ImGui::CalcTextSize(lampLang::LS("LAMPRAY_DEPLOY")).x +
           style.FramePadding.x * 2.0f;
    ...
    size = ImGui::CalcTextSize(lampLang::LS("LAMPRAY_RESET")).x +
           style.FramePadding.x * 2.0f;
    avail = ImGui::GetContentRegionAvail().x;
    ...
    if (ImGui::Button(lampLang::LS("LAMPRAY_RESET"))) {
        Lamp::Core::FS::lampTrack::reset(
            Lamp::Games::getInstance().currentGame->Ident().ReadableName);
        std::filesystem::path installPath(
            Lamp::Games::getInstance()
                .currentGame->KeyInfo()["installDirPath"]);
        system(("pkexec umount \"" +
                Lamp::Games::getInstance()
                    .currentGame->KeyInfo()["installDirPath"] +
                "\"")
                   .c_str());
        std::filesystem::rename(
            installPath.parent_path() /
                ("Lampray Managed - " + installPath.stem().string()),
            Lamp::Games::getInstance()
                .currentGame->KeyInfo()["installDirPath"]);
        system(
            ("pkexec umount \"" +
             Lamp::Games::getInstance().currentGame->KeyInfo()["appDataPath"] +
             "/Mods\"")
                .c_str());
        std::filesystem::rename(
            std::filesystem::path(Lamp::Games::getInstance()
                                      .currentGame->KeyInfo()["appDataPath"] +
                                  "/Mods")
                    .parent_path() /
                ("Lampray Managed - " +
                 std::filesystem::path(
    ...
    if (deployCheck) {
        if (!Lamp::Games::getInstance().currentGame->installPathSet()) {
            Lamp::Core::lampNotification::getInstance().pushNotification(
                "warning",
                Lamp::Games::getInstance().currentGame->Ident().ReadableName +
                    Lamp::Core::lampLang::getInstance().LS(
                        "LAMPRAY_WARN_GAME_PATH"),
                true);
        }
        ...
        ImGui::Begin(
            lampLang::LS("LAMPRAY_CHECK"), NULL,
            Lamp::Core::lampConfig::getInstance().DefaultWindowFlags());
        Lamp::Core::lampNotification::getInstance().DisplayNotifications();
        ...
        if (ImGui::Button(lampLang::LS("LAMPRAY_START"))) {
            ...
            std::thread([] {
                Lamp::Core::Base::LampSequencer::add(
                    "Deployment Start", []() -> lampReturn {
                        Lamp::Core::FS::lampTrack::reset(
                            Lamp::Games::getInstance()
                                .currentGame->Ident()
                                .ReadableName);

                        return Lamp::Core::Base::lampLog::getInstance().pLog(
                            {1, "Task Complete."});
                    });
                Lamp::Core::Base::LampSequencer::add(
                    "Deployment Start", []() -> lampReturn {
                        Lamp::Games::getInstance()
                            .currentGame->startDeployment();
                        return Lamp::Core::Base::lampLog::getInstance().pLog(
                            {1, "Task Complete."});
                    });
        ...
        if (ImGui::BeginMenu(
            ...
            if (ImGui::BeginMenu(lampLang::LS("LAMPRAY_GAMEPICK"))) {
                ...
                for (Game::gameControl *element :
                    ...
                    if (ImGui::MenuItem(element->Ident().ReadableName)) {
                        ...
                        Lamp::Core::FS::lampIO::saveKeyData(
                            "PreviousGame", std::to_string(gameCount),
                            "LAMP CONFIG");
                    }
                    gameCount++;
                }
            ...
            if (currentMenu == GAME_MOD_MENU) {
                ...
                if (ImGui::BeginMenu(lampLang::LS("LAMPRAY_PROFSELECT"))) {
                    ...
                    for (std::string x :
                         Lamp::Core::Base::lampMod::Profile::splitString(
                             Lamp::Games::getInstance()
                                 .currentGame->KeyInfo()["ProfileList"])) {

                        if (x == Lamp::Games::getInstance().currentProfile) {
                            if (ImGui::BeginMenu(("* " + x + " *").c_str())) {
                                if (ImGui::MenuItem(
                                        lampLang::LS("LAMPRAY_PROFLOAD"))) {
                                    Lamp::Games::getInstance().currentProfile =
                                        x;
                                    Lamp::Games::getInstance()
                                        .currentGame->getModList()
                                        .clear();
                                    Lamp::Games::getInstance()
                                        .currentGame->getModList() =
                                        FS::lampIO::loadModList(
                                            Lamp::Games::getInstance()
                                                .currentGame->Ident()
                                                .ShortHand,
                                            x);
                                    FS::lampIO::saveKeyData(
                                        "CurrentProfile", x,
                                        Lamp::Games::getInstance()
                                            .currentGame->Ident()
                                            .ShortHand);
                            ...
                            if (ImGui::BeginMenu(x.c_str())) {
                                ...
                                if (x != "Default") {
                                    if (ImGui::MenuItem(
                                            lampLang::LS("LAMPRAY_PROFDEL"))) {
                                        Lamp::Core::Base::lampMod::Profile::
                                            removeValue(
                                                Lamp::Games::getInstance()
                                                    .currentGame
                                                    ->KeyInfo()["ProfileList"],
            ...
            if (ImGui::BeginMenu(lampLang::LS("LAMPRAY_ABT"))) {
                ...
                ImGui::MenuItem("--------------------------------------");
                ImGui::MenuItem(lampLang::LS("LAMPRAY_ABT1") +
...
void Lamp::Core::lampMenu::createProfileDialog() {
    ...
    if (ImGui::InputText("##inputProf", profileBuffer, 250,
                         ImGuiInputTextFlags_EnterReturnsTrue) ||
        ImGui::Button(lampLang::LS("LAMPRAY_PROFCRES"))) {
        std::string pb(profileBuffer);
        Lamp::Core::Base::lampMod::Profile::addValue(
        ...
        if (pfCopyStr != "" && pfCopy) {
            if (Lamp::Games::getInstance().currentProfile != pfCopyStr) {
                Lamp::Games::getInstance()
                    .currentGame->getModList() = FS::lampIO::loadModList(
                    Lamp::Games::getInstance().currentGame->Ident().ShortHand,
                    pfCopyStr);
            }
            FS::lampIO::saveModList(
                Lamp::Games::getInstance().currentGame->Ident().ShortHand,
                Lamp::Games::getInstance().currentGame->getModList(), pb);
        }
        Lamp::Games::getInstance().currentGame->getModList() =
            FS::lampIO::loadModList(
                Lamp::Games::getInstance().currentGame->Ident().ShortHand, pb);
